...
1. You should create a topic and a queue. Open the `queues.conf` and `topics.conf` files and append to them the lines containing`portfolioQueue` for `queues.conf` and `portfolioTopic` for `topics.conf`.
